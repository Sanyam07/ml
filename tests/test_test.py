...
def test_score_mse():
    ...
    y_true[:5] = 1
    y_pred = np.arange(1, 11)
    assert test.score_mse(y_true, y_pred) == 3.5

    y_true.mask = np.zeros(10)
    y_true.mask[:5] = 1
    assert test.score_mse(y_true, y_pred) == 1

    assert test.score_mse(np.arange(0, 10), y_pred) == 1
    assert test.score_mse(np.arange(0, 5), np.arange(3, 8)) == 9
...
def test_score_mae():
    ...
    y_true[:5] = 1
    y_pred = np.arange(1, 11)
    assert test.score_mae(y_true, y_pred) == 1.5

    y_true.mask = np.zeros(10)
    y_true.mask[:5] = 1
    assert test.score_mae(y_true, y_pred) == 1

    assert test.score_mae(np.arange(0, 10), y_pred) == 1
    assert test.score_mae(np.arange(0, 5), np.arange(3, 8)) == 3


def test_score_r2():
    assert test.score_r2(np.zeros(5), np.zeros(5)) == 1.0
    assert test.score_r2(np.arange(5), np.arange(5)) == 1.0

    y_true = np.ma.arange(0, 10)
    y_true[:5] = 1
    y_pred = np.arange(1, 11)
    assert test.score_r2(y_true, y_pred) - 0.65 < 1e-5

    y_true.mask = np.zeros(10)
    y_true.mask[:5] = 1
    assert test.score_r2(y_true, y_pred) == 0.5

    assert test.score_r2(np.arange(0, 10), y_pred) - 0.87878 < 1e-5
    assert test.score_r2(np.arange(0, 5), np.arange(3, 8)) == -3.5
...
def test_score_ca():
    assert test.score_ca(np.zeros(5), np.zeros(5)) == 1.0
    assert test.score_ca(np.zeros(5), np.ones(5)) == 0.0

    y_true = np.ma.arange(0, 10)
    y_true.mask = np.zeros(10)
    y_true.mask[:5] = 1
    y_pred = np.arange(0, 10)
    assert test.score_ca(y_true, y_pred) == 1.0
    y_pred[-1] = 1
    assert test.score_ca(y_true, y_pred) == 0.8
